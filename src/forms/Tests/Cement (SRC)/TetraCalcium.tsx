...
const TetraCalcium = () => {
    const [c4a, setc4a] = useState<number>(0);
    const [c3a, setc3a] = useState<number>(0);
            ...
            if (job && job.bench_record) {
                ...
                const getRes = async () => {
                    try {
                        ...
                        // Ensure C4AF_C3A exists before destructuring
                        if (benchRec[0].C4AF_C3A) {
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        // const resultObj = { c4a, c3a };
        const Tc = ((c4a) + (2 * c3a)).toFixed(2);

        const updatedBenchRecord = [{ C4AF_C3A: { c4a, c3a } }];
        const updatedReportValues = [{ C4AF_C3A: { Tc } }];
    ...
    return (
        ...
        >
            <h3 style={{ textAlign: 'center', marginBottom: '20px', fontWeight: 'bold' }}>TetraCalcium Calculation</h3>

            {renderInput("C4AF", "input1", c4a, setc4a)}
            {renderInput("C3A", "input2", c3a, setc3a)}
            ...
            <div style={{ marginTop: '20px', fontWeight: 'bold' }}>
                <p>(C4AF + 2C3A) = C4A + 2C3A</p>
                <p>
                    (C4AF + 2C3A) = {c4a} + 2({c3a})
                </p>
                <p>(C4AF + 2C3A) = {((c4a) + (2 * c3a)).toFixed(2)}</p>
            </div>
        </form>
    );
};
