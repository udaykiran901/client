...
const DeterminationOfChloride = () => {
            ...
            if (job && job.bench_record) {
                ...
                const getRes = async () => {
                    try {
                        ...
                        // Ensure cl and resultObj exist before destructuring
                        if (benchRec[0].cl && benchRec[0].cl.resultObj) {
                            const { n, w, y, x, z } = benchRec[0].cl.resultObj;
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const resultObj = { n, w, y, x, z };
        const cl = (((10 - (10 - y) - x) * n * 0.03545 * 100) / w).toFixed(2);
    ...
    return (
        ...
        >
            <h3 style={{ textAlign: 'center', marginBottom: '20px', fontWeight: 'bold' }}>Determination of Chloride</h3>

            {renderInput("Normality of AgNO3 (N)", "input1", n, setN)}
            {renderInput("Weight of the Sample (W)", "input2", w, setW)}
            {renderInput("ml of NH4SCN for blank (Y)", "input3", y, setY)}
            {renderInput("ml of NH4SCN for Sample (X)", "input4", x, setX)}
            ...
            <div style={{ marginTop: '20px', fontWeight: 'bold' }}>
                <p>Cl % = ((Z * N * 0.03545 * 100)/ (W))</p>
                <p>
                    Cl % = {(((10 - (10 - y) - x) * n * 0.03545 * 100) / w).toFixed(2)}
                </p>
            </div>
        </form>
    );
...
export default DeterminationOfChloride;
