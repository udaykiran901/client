...
const CWChlorides = () => {
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const os = (((f - i - b) * n * 35.45 * 1000) / v1).toFixed();

        const updatedBenchRecord = [{ CWChlorides: { v1, b, f, i, n } }];
        const updatedReportValues = [{ CWChlorides: { os } }];
    ...
    return (
        ...
        >
            <h3 style={{ textAlign: 'center', marginBottom: '20px', fontWeight: 'bold' }}>
                Chlorides Calculation
            </h3>

            {renderInput("Volume of Sample (v1)", "input1", v1, setV1)}
            {renderInput("Volume of AgNO3 Consumed for Blank (B)", "input2", b, setB)}
            {renderInput("Normality of AgNO3 (N)", "input3", n, setN)}
            {renderInput("Volume of AgNO3 Consumed Final (F)", "input4", f, setF)}
            {renderInput("Volume of AgNO3 Consumed Initial (I)", "input5", i, setI)}
            ...
            <div style={{ marginTop: '20px', fontWeight: 'bold' }}>
                <p>Chlorides as Cl = (((F - I) - B) * N * 35.45 * 1000) / v1</p>
                <p>Chlorides as Cl = (({f} - {i} - {b}) * {n} * 35.45 * 1000) / {v1}</p>
                <p>Chlorides as Cl = {(((f - i - b) * n * 35.45 * 1000) / v1).toFixed()}</p>
            </div>
        </form>
    );
};
