...
const CWInOrganicSolids = () => {
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const os = (((w3 - w1) * (10 ** 6)) / ws).toFixed();

        const updatedBenchRecord = [{ CWOrganicSolids: { ws, w1, w3 } }];
        const updatedReportValues = [{ CWOrganicSolids: { os } }];
    ...
    return (
        ...
        >
            ...
            {renderInput("Wt of glass dish (W1)", "input2", w1, setW1)}
            {renderInput("Wt. of glass dish after evaporation (W3)", "input3", w3, setW3)}
            ...
            <div style={{ marginTop: '20px', fontWeight: 'bold' }}>
                <p>InOrganic Solids = ((W3 - W1) * 10^6) / Ws</p>
                <p>
                    InOrganic Solids = (({w3} - {w1}) * 10^6) / {ws} mg/l
                </p>
                <p>InOrganic Solids = {(((w3 - w1) * (10 ** 6)) / ws).toFixed()} mg/l</p>
            </div>
        </form>
    );
};
...
export default CWInOrganicSolids;
