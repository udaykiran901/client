...
const CWSulphate = () => {
            ...
            if (job && job.bench_record) {
                ...
                const getRes = async () => {
                    try {
                        ...
                        // Ensure SulphateTest and required properties exist before destructuring
                        if (benchRec[0].SulphateTest) {
                            const { ws, w1, w2 } = benchRec[0].SulphateTest;
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const os = (((w2 - w1) * 0.343 * 10 ** 6) / ws).toFixed();

        const updatedBenchRecord = [{ SulphateTest: { ws, w1, w2 } }];
        const updatedReportValues = [{ SulphateTest: { os } }];
    ...
    return (
        ...
        >
            <h3 style={{ textAlign: 'center', marginBottom: '20px', fontWeight: 'bold' }}>Sulphate Calculation</h3>

            {renderInput("vol. of Sample (Ws)", "input1", ws, setWs)}
            {renderInput("Empty Cruicible (w1)", "input2", w1, setW1)}
            {renderInput("Cruicible + PPT (w2)", "input3", w2, setW2)}
            ...
            <div style={{ marginTop: '20px', fontWeight: 'bold' }}>
                <p>SO3 % = ((w2 - w1) * 0.343 * 10^6) / ws</p>
                <p>
                    SO3 % = (({w2} - {w1}) * 0.343 * 10^6) / {ws} mg/l
                </p>
                <p>SO3 % = {(((w2 - w1) * 0.343 * 10 ** 6) / ws).toFixed()} mg/l</p>
            </div>
        </form>
    );
};
