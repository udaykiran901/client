...
const DWTotalDissolvedSolids = () => {
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const os = Math.round(((w2 - w1) * 10 ** 6) / v1);

        const updatedBenchRecord = [{ TDS: { v1, w1, w2 } }];
        const updatedReportValues = [{ TDS: { os } }];
    ...
    return (
        ...
        >
            <h3 style={{ textAlign: 'center', marginBottom: '20px', fontWeight: 'bold' }}>Total Dissolved Solids Calculation</h3>

            {renderInput("Vol. of Sample (vs)", "input1", v1, setV1)}
            {renderInput("Weight of Glass dish (W1)", "input2", w1, setW1)}
            {renderInput("Weight after evaporation (W2)", "input3", w2, setW2)}
            ...
            <div style={{ marginTop: '20px', fontWeight: 'bold' }}>
                <p>TDS mg/l = ((W2 - W1) * 10^6) / Vs</p>
                <p>
                    TDS mg/l = (({w2} - {w1}) * 10^6) / {v1}
                </p>
                <p>TDS mg/l = {Math.round(((w2 - w1) * 10 ** 6) / v1)}</p>
            </div>
        </form>
    );
};
...
export default DWTotalDissolvedSolids;
