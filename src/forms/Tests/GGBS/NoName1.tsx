...
const DeterminationOfNoName1 = () => {
    ...
    useEffect(() => {
        if (singleJob.length > 0 && review) {
            ...
            if (job && job.bench_record) {
                ...
                const getRes = async () => {
                    try {
                        console.log(benchRec, 'raw bench content');

                        // Ensure that all properties exist before setting state
                        const { ca, mg, al, si } = benchRec[0];
                        if (ca !== undefined && mg !== undefined && al !== undefined && si !== undefined) {
                            setCa(ca);
                            setMg(mg);
                            setAl(al);
                            setSi(si);
                        ...
                        console.log("Error accessing bench record properties:", err);
                    }
                };
                getRes();
            }
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const resultObj = { ca, mg, al, si };
        const val = ((ca + mg + (al / 3)) / (si + ((2 * al) / 3))).toFixed(2);

        const updatedBenchRecord = [{ ca, mg, al, si }];
        const updatedReportValues = [{ val }];
    ...
    return (
        ...
        >
            ...
            <div style={{ marginTop: '20px', fontWeight: 'bold' }}>
                <p>Percentage of major Oxides = (CaO + MgO + (Al2O3/3)) / (SiO2 + (2*Al2O3)/3)</p>
                <p>
                    Percentage of major Oxides = ({ca} + {mg} + ({al}/3)) / ({si} + ((2*{al})/3))
                </p>
                <p>Percentage of major Oxides = {((ca + mg + (al / 3)) / (si + ((2 * al) / 3))).toFixed(2)}</p>
            </div>
        </form>
    );
};
...
export default DeterminationOfNoName1;
