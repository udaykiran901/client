...
const FreeWater = () => {
            ...
            if (job && job.bench_record) {
                ...
                const getRes = async () => {
                    try {
                        ...
                        // Ensure benchRec[0] exists and contains w1, w2, w3 before destructuring
                        if (benchRec[0] && 'w1' in benchRec[0] && 'w2' in benchRec[0] && 'w3' in benchRec[0]) {
                            const { w1, w2, w3 } = benchRec[0];
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const FW = (((w2 - w3) * 100) / (w2 - w1)).toFixed(2);

        const updatedBenchRecord = [{ w1, w2, w3 }];
        const updatedReportValues = [{ FW }];
    ...
    return (
        ...
        >
            <h3 style={{ textAlign: "center", marginBottom: "20px", fontWeight: "bold" }}>Free Water Calculation</h3>
            ...
            <div style={{ marginTop: "20px", fontWeight: "bold" }}>
                <p>F.W % = (((w2 - w3) * 100) / (w2 - w1))</p>
                <p>F.W % = ((({w2} - {w3}) * 100) / ({w2} - {w1}))</p>
                <p>F.W % = {(((w2 - w3) * 100) / (w2 - w1)).toFixed(2)}</p>
            </div>
        </form>
    );
};
