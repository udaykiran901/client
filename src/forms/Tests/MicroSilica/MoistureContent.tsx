...
const MoistureContent = () => {
            ...
            if (job && job.bench_record) {
                ...
                const getRes = async () => {
                    try {
                        ...
                        // Ensure MoistureContent exists before destructuring
                        if (benchRec[0].MoistureContent) {
                            const { w1, w2, w3 } = benchRec[0].MoistureContent;
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const mc = (((w2 - w3) * 100) / (w2 - w1)).toFixed(2);

        const updatedBenchRecord = [{ MoistureContent: { w1, w2, w3 } }];
        const updatedReportValues = [{ MoistureContent: { mc } }];
    ...
    return (
        ...
        >
            <h3 style={{ textAlign: 'center', marginBottom: '20px', fontWeight: 'bold' }}>Moisture Content Calculation</h3>

            {renderInput("W1", "input1", w1, setW1)}
            {renderInput("W2", "input2", w2, setW2)}
            {renderInput("W3", "input3", w3, setW3)}
            ...
            <div style={{ marginTop: '20px', fontWeight: 'bold' }}>
                <p>Moisture Content % = (((W2 - W3) * 100) / (W2 - W1))</p>
                <p>
                    Moisture Content % = ((({w2} - {w3}) * 100) / ({w2} - {w1}))
                </p>
                <p>Moisture Content % = {(((w2 - w3) * 100) / (w2 - w1)).toFixed(2)}</p>
            </div>
        </form>
    );
};
