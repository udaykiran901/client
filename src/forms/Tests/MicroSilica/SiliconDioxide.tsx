...
const SiliconDioxide = () => {
            ...
            if (job && job.bench_record) {
                ...
                const getRes = async () => {
                    try {
                        ...
                        if (benchRec[0] && benchRec[0].SiO2) {
                            const { w, w1, w2, w3 } = benchRec[0].SiO2;
    ...
    const handleOnSubmittingTest = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const sd = (((w2 - w3) / w) * 100).toFixed(2);
        const updatedBenchRecord = [{ SiO2: { w, w1, w2, w3 } }];
        const updatedReportValues = [{ SiO2: { sd } }];
    ...
    return (
        ...
        >
            <h3 style={{ textAlign: 'center', marginBottom: '20px', fontWeight: 'bold' }}>Silicon Dioxide Calculation</h3>

            {renderInput("Ws", "input1", w, setW)}
            {renderInput("W1", "input2", w1, setW1)}
            {renderInput("W2", "input3", w2, setW2)}
            {renderInput("W3", "input4", w3, setW3)}
            ...
            <div style={{ marginTop: '20px', fontWeight: 'bold' }}>
                <p>SiO2 % = (((W2 - W3) / Ws) * 100)</p>
                <p>
                    SiO2 % = ((({w2} - {w3}) / {w}) * 100)
                </p>
                <p>SiO2 % = {(((w2 - w3) / w) * 100).toFixed(2)}</p>
            </div>
        </form>
    );
};
